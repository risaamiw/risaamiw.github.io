...
class Users extends CI_Model
{
	...
	function activate_user_manual($user_id, $activate_by_email)
	{
		$this->db->select('1', FALSE);
		$this->db->where('id', $user_id);
		$this->db->where('activated', 0);
	...
	public function check_ldap($data)
	{
	...
	function reset_password_ldap($username, $new_pass, $expire_period = 900)
	{
		$this->db->set('password', $new_pass);
